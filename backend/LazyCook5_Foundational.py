...
# IMPORTANT for backend imports:
# This file may be imported by FastAPI/uvicorn. Re-wrapping sys.stdout/sys.stderr at import time can break
# server processes on Windows ("lost sys.stderr"). So we only do this in CLI mode.
if sys.platform == "win32" and os.getenv("LAZYCOOK_CLI", "0") == "1":
    try:
        sys.stdout = io.TextIOWrapper(sys.__stdout__.buffer, encoding="utf-8", errors="replace", line_buffering=True)
        sys.stderr = io.TextIOWrapper(sys.__stderr__.buffer, encoding="utf-8", errors="replace", line_buffering=True)
    except Exception:
        pass
...
class AnimatedProgressBar(ProgressBar):
    ...
    def __init__(self, style="green", pulse_style="grey50"):
        ...
        self._animation_frames = ["â ‹", "â ™", "â ¹", "â ¸", "â ¼", "â ´", "â ¦", "â §", "â ‡", "â "]
        self._frame_index = 0
...
class TextFileManager:
    def __init__(self, data_dir: str = "multi_agent_data", conversation_limit: int = 70):
        ...
        self.max_storage_per_user = 500 * 1024 * 1024 #500MB
    ...
    def get_conversation_context(self, user_id: str, limit: int = None) -> str:
        ...
        # Build fresh context - DOCUMENTS FIRST
        context_parts = ["=== ðŸ“œ CONVERSATION CONTEXT ==="]

        # ADD DOCUMENTS FIRST (MOST IMPORTANT)
        documents = self.get_user_documents(user_id, 50)
        if documents:
            context_parts.append("\n=== ðŸ“„ UPLOADED DOCUMENTS (FULL CONTENT) ===")
            for i, doc in enumerate(documents):
                context_parts.append(f"\n--- Document {i + 1}: {doc.filename} ---")
                context_parts.append(doc.content)  # FULL CONTENT, NO TRUNCATION
            context_parts.append("\n=== END OF DOCUMENTS ===")
        ...
        if conversations:
            ...
            for i, conv in enumerate(conversations):
                ...
                context_parts.append(f"ðŸ’¤ USER: {conv.user_message}")
                context_parts.append(f"ðŸ¤– ASSISTANT: {conv.ai_response}")
        ...
        # DEBUG LOG
        logger.info(f"Context built: {len(context)} chars, {len(documents)} documents included")
    ...
    def get_documents_context(self, user_id: str, limit: int = 50, full_content: bool = True) -> str:
        ...
        for i, doc in enumerate(documents):
            ...
            if full_content:
                # CHANGED: No truncation, pass complete document
                doc_tokens = len(doc.content.split()) * 1.3  # Rough token estimate

                if doc_tokens < 100000:  # Under ~75K tokens, safe for Gemini 2.0
                    context_parts.append(doc.content)
                else:
                    # Only truncate truly massive docs
                    context_parts.append(doc.content[:500000])
                    context_parts.append("\n[Document truncated - exceeds 500K characters]")
            else:
    ...
    def process_uploaded_file(self, file_path: str, user_id: str) -> Optional[Document]:
        try:
            ...
            if file_type.startswith('text/'):
                ...
                try:
                    ...
                    for i in range(number_of_pages):
                        ...
                        if text.strip():
                            # FIX: Remove surrogate characters
            ...
            # FIX: Clean content one more time before saving
    ...
    @log_errors
    def get_conversation_context_smart(self, user_id: str, limit: int = None, include_full_docs: bool = False) -> str:
        """Smart context that can exclude full document content to save tokens"""
        limit = self._get_effective_limit(limit)

        # Check cache validity
        cache_key = f"{user_id}_{limit}_{include_full_docs}"
        now = datetime.now()
        ...
        # ALWAYS add documents FIRST (most important)
        if include_full_docs:
            ...
            docs_summary = self.get_documents_summary(user_id)
            if docs_summary:
                context_parts.append("\n--- ðŸ“„ UPLOADED DOCUMENTS (SUMMARIES) ---")
                context_parts.append(docs_summary)
        ...
        if conversations:
            ...
            for i, conv in enumerate(conversations):
                ...
                # Truncate old responses to save tokens
                response_preview = conv.ai_response[:500] + "..." if len(conv.ai_response) > 500 else conv.ai_response
                context_parts.append(f"ðŸ¤– ASSISTANT: {response_preview}")
    ...
    @log_errors
    def get_documents_summary(self, user_id: str) -> str:
        """Get document list with summaries instead of full content"""
        documents = self.get_user_documents(user_id, 50)
        if not documents:
            return ""

        summary_parts = [
            "\nâš ï¸ DOCUMENTS AVAILABLE (Use 'Full Documents' mode to access complete content):\n"
        ]
        ...
        for i, doc in enumerate(documents):
            ...
            summary_parts.append(
                f"\nðŸ“„ {doc.filename} ({len(doc.content)} characters, {doc.file_size} bytes)"
                f"\nType: {doc.file_type}"
                f"\nPreview: {preview}"
                f"\n[Full content available - select 'Full Documents' mode to access]\n"
            )
    ...
    def _format_markdown_export(self, conversations: List[Conversation]) -> str:
        ...
        lines = []
        lines.append("# ðŸ”¥ LAZYCOOK Chat Export")
...
class DocumentChunk:
    ...
    def __init__(self, chunk_id: str, doc_id: str, content: str,
                 chunk_index: int, embedding: np.ndarray = None):
...
class DocumentEmbeddingStore:
    """
    Production-ready document embedding with:
    - Fixed chunk size + overlap
    - Versioned vector indexes
    - Separate ingest/query pipelines
    - Duplicate detection
    """
    ...
    def __init__(self, data_dir: str = "multi_agent_data"):
        ...
        # Fixed parameters (DO NOT change during runtime)
        self.chunk_size = 1024  # 2x larger chunks
        self.chunk_overlap = 256  # Keep 25% overlap
        ...
        if SentenceTransformer is None:
            raise ImportError(
                "sentence_transformers is not installed. Install it (and its torch dependency) to enable embeddings."
            )
        self.model = SentenceTransformer('all-MiniLM-L6-v2')

        # Version tracking
        self.current_version = "v1"
        self.indexes = {}  # {version: faiss_index}
        self.chunk_metadata = {}  # {version: {chunk_id: metadata}}

        # Load existing indexes
        self._load_indexes()

    def _load_indexes(self):
        """Load all versioned indexes from disk"""
        for version_dir in self.embeddings_dir.glob("repo_v*"):
            ...
            if index_path.exists() and metadata_path.exists():
                try:
                    if faiss is None:
                        raise ImportError("faiss is not installed. Install faiss-cpu to enable vector index loading.")
                    self.indexes[version] = faiss.read_index(str(index_path))
                    with open(metadata_path, 'rb') as f:
                        self.chunk_metadata[version] = pickle.load(f)
                    logger.info(f"Loaded {version} with {self.indexes[version].ntotal} vectors")
                except Exception as e:
                    logger.error(f"Failed to load {version}: {e}")

    def _save_index(self, version: str):
        ...
        if faiss is None:
            raise ImportError("faiss is not installed. Install faiss-cpu to enable vector index saving.")
        faiss.write_index(self.indexes[version], str(index_path))
        with open(metadata_path, 'wb') as f:
            pickle.dump(self.chunk_metadata[version], f)

        logger.info(f"Saved {version} with {self.indexes[version].ntotal} vectors")

    def _chunk_text(self, text: str) -> List[str]:
        """
        Chunk text with fixed size and overlap.
        Uses character-based approximation: 1 token â‰ˆ 4 chars
        """
        chunk_chars = self.chunk_size * 4  # 512 tokens â‰ˆ 2048 chars
        overlap_chars = self.chunk_overlap * 4  # 128 tokens â‰ˆ 512 chars
        ...
        while start < len(text):
            ...
            if end < len(text):
                ...
                if break_point > chunk_chars * 0.7:  # At least 70% of chunk
    ...
    def _detect_duplicate(self, doc_hash: str, version: str) -> bool:
        """Check if document already indexed by hash"""
        if version not in self.chunk_metadata:
            return False

        for chunk_meta in self.chunk_metadata[version].values():
            if chunk_meta.get('doc_hash') == doc_hash:
                return True
        return False

    def ingest_document(self, document: Document, user_id: str) -> dict:
        """
        INGEST PIPELINE (separate from query)
        Steps:
        1. Check for duplicates
        2. Chunk document
        3. Generate embeddings
        4. Store in versioned index
        """
        version = self.current_version

        # 1. Duplicate detection
        if self._detect_duplicate(document.hash_value, version):
            logger.info(f"Document {document.filename} already indexed (duplicate hash)")
            return {
                'status': 'duplicate',
                'doc_id': document.id,
                'chunks': 0
        ...
        if version not in self.indexes:
            # Create new index for this version
            if faiss is None:
                raise ImportError("faiss is not installed. Install faiss-cpu to enable vector indexing.")
            self.indexes[version] = faiss.IndexFlatL2(self.embedding_dim)
            self.chunk_metadata[version] = {}

        # Add vectors to index
        start_id = self.indexes[version].ntotal
        self.indexes[version].add(embeddings)
        ...
        for i, chunk in enumerate(chunks):
            ...
            self.chunk_metadata[version][vector_id] = {
                'chunk_id': chunk_id,
                'doc_id': document.id,
                'doc_hash': document.hash_value,
    ...
    def query_documents(self, query: str, user_id: str, top_k: int = 5,
                        version: str = None) -> List[dict]:
        """
        QUERY PIPELINE (separate from ingest)
        Steps:
        1. Embed query
        2. Search versioned index
        3. Filter by user_id
        ...
        if version not in self.indexes:
            logger.warning(f"No index found for version {version}")
            return []

        # 1. Embed query
        query_embedding = self.model.encode([query])[0].astype('float32')

        # 2. Search index (get more candidates for filtering)
        distances, indices = self.indexes[version].search(
            query_embedding.reshape(1, -1),
            top_k * 5  # Get 5x candidates for user filtering
        )

        # 3. Filter by user_id and rank
        results = []
        for distance, idx in zip(distances[0], indices[0]):
            if idx == -1:  # FAISS returns -1 for no match
                continue

            metadata = self.chunk_metadata[version].get(int(idx))
            if metadata and metadata['user_id'] == user_id:
                results.append({
                    ...
                    'relevance_score': 1.0 / (1.0 + float(distance))  # Convert to similarity
                })
    ...
    def get_stats(self, version: str = None) -> dict:
        ...
        for metadata in self.chunk_metadata[version].values():
            unique_docs.add(metadata['doc_id'])
            unique_users.add(metadata['user_id'])

        return {
            'version': version,
            'total_chunks': self.indexes[version].ntotal,
...
# ============================================================================
# 6ï¸âƒ£ CACHING LAYER - Latency + Cost Multiplier
# ============================================================================

class TTLCache:
    """Time-to-live cache with different expiration policies"""

    # TTL Classes in seconds
    TTL_CLASSES = {
        'no_cache': 0,  # Never cache (auth, payments)
        'realtime': 60,  # 1 minute (stock prices, weather)
        'stable': 3600,  # 1 hour (news, articles)
        'reference': 86400,  # 1 day (documentation)
        'permanent': 2592000  # 30 days (static content)
    }

    # NO-CACHE patterns (never cache these)
    NO_CACHE_PATTERNS = [
        'auth', 'login', 'password', 'token', 'payment',
        'credit_card', 'private', 'secret', 'api_key'
    ]
    ...
    def _should_cache(self, content: str) -> bool:
        """Check if content should be cached based on NO-CACHE patterns"""
        content_lower = content.lower()
        return not any(pattern in content_lower for pattern in self.NO_CACHE_PATTERNS)

    def generate_cache_key(self, query: str, context_hash: str,
                           agent_version: str, model_params: dict) -> str:
        """
        Generate deterministic cache key.
        Formula: hash(query + context_hash + agent_version + params)
        """
        cache_input = {
    ...
    def get(self, cache_key: str) -> Optional[Any]:
        ...
        if cache_key in self.expiry:
            if datetime.now() > self.expiry[cache_key]:
                # Expired
        ...
        self.hits += 1
        logger.info(f"Cache HIT: {cache_key[:16]}... (hit rate: {self.hit_rate():.1%})")
        return self.cache[cache_key]

    def set(self, cache_key: str, value: Any, ttl_class: str = 'stable'):
        """Set value in cache with TTL"""

        # Check if should cache
        if isinstance(value, str) and not self._should_cache(value):
            logger.warning(f"Refusing to cache sensitive content")
            return False

        ttl_seconds = self.TTL_CLASSES.get(ttl_class, self.TTL_CLASSES['stable'])
        ...
        self.expiry[cache_key] = datetime.now() + timedelta(seconds=ttl_seconds)

        logger.info(f"Cache SET: {cache_key[:16]}... (TTL: {ttl_class})")
        return True
    ...
    def invalidate(self, pattern: str = None):
        ...
        if pattern is None:
            ...
            for k in to_delete:
                ...
                if k in self.expiry:
                    del self.expiry[k]
            logger.info(f"Cache invalidated: {len(to_delete)} entries matching '{pattern}'")
    ...
    def stats(self) -> dict:
        ...
        expired = sum(1 for exp in self.expiry.values() if datetime.now() > exp)
...
# ============================================================================
# 9ï¸âƒ£ ENHANCED TASK SYSTEM - Production Ready
# ============================================================================

@dataclass
class EnhancedTask:
    """Production-ready task with all required fields"""
    # Core fields
    id: str
    conversation_id: str
    task_type: str
    ...
    # State management
    state: str  # 'pending', 'running', 'completed', 'failed', 'retry'
    assigned_agents: List[str]  # Which agents will/did handle this

    # Context tracking
    input_refs: List[str]  # Document IDs used as input
    context_hash: str  # Hash of context used

    # Completion tracking
    completion_criteria: Dict[str, Any]  # What defines success
    failure_conditions: List[str]  # What defines failure
...
# ============================================================================
# INTEGRATION: Enhanced TextFileManager with Embedding Store
# ============================================================================

class EnhancedTextFileManager(TextFileManager):
    """
    Drop-in replacement for TextFileManager with:
    ...
    def __init__(self, data_dir: str = "multi_agent_data", conversation_limit: int = 70):
        super().__init__(data_dir, conversation_limit)

        # Initialize new components
        # Embeddings are optional in backend mode (keeps API usable without torch/sentence-transformers/faiss).
        # If dependencies are missing, we degrade gracefully (semantic search disabled; full-doc context still works).
        try:
            self.embedding_store = DocumentEmbeddingStore(data_dir)
        except Exception as e:
            logger.warning(f"Embeddings disabled (DocumentEmbeddingStore init failed): {e}")
            self.embedding_store = None
        self.cache = TTLCache(data_dir)

        # Enhanced task file
        self.enhanced_tasks_file = self.data_dir / "enhanced_tasks.json"
        if not self.enhanced_tasks_file.exists():
            self.enhanced_tasks_file.write_text("[]")

    def process_uploaded_file(self, file_path: str, user_id: str) -> Optional[Document]:
        """
        Override to add embedding generation.
        Steps:
        1. Process file (original method)
        2. Generate embeddings
        3. Store in vector index
        """
        # 1. Original processing
        document = super().process_uploaded_file(file_path, user_id)
        ...
        try:
            if self.embedding_store is None:
                document.metadata['embedding_status'] = 'disabled'
                document.metadata['embedding_error'] = 'Embeddings disabled'
                return document
            logger.info(f"Generating embeddings for {document.filename}...")
            ingest_result = self.embedding_store.ingest_document(document, user_id)

            # Update document metadata with embedding info
            document.metadata['embedding_status'] = ingest_result['status']
            document.metadata['chunks'] = ingest_result.get('chunks', 0)
            document.metadata['embedding_version'] = ingest_result.get('version')

            # Re-save with updated metadata
            self.save_document(document)

            logger.info(f"Embeddings created: {ingest_result}")
        except Exception as e:
            logger.error(f"Failed to generate embeddings: {e}")
            document.metadata['embedding_status'] = 'failed'
            document.metadata['embedding_error'] = str(e)

        return document

    def get_relevant_document_chunks(self, query: str, user_id: str, top_k: int = 5) -> str:
        """
        NEW METHOD: Get relevant document chunks using semantic search.
        This replaces dumping full documents into context.
        """
        try:
            if self.embedding_store is None:
                return ""
            chunks = self.embedding_store.query_documents(query, user_id, top_k)
            ...
            for i, chunk in enumerate(chunks, 1):
                context_parts.append(f"\n--- Excerpt {i} from {chunk['filename']} ---")
                context_parts.append(f"Relevance: {chunk['relevance_score']:.2%}")
    ...
    def save_enhanced_task(self, task: EnhancedTask):
        """Save enhanced task"""
        tasks = self._read_json_file(self.enhanced_tasks_file)
        tasks = [t for t in tasks if t.get('id') != task.id]
        tasks.append(task.to_dict())
        tasks.sort(key=lambda x: x.get('created_at', ''), reverse=True)
        self._write_json_file(self.enhanced_tasks_file, tasks)

    def get_enhanced_task(self, task_id: str) -> Optional[EnhancedTask]:
        """Get enhanced task by ID"""
        tasks = self._read_json_file(self.enhanced_tasks_file)
        for task_data in tasks:
            if task_data.get('id') == task_id:
                return EnhancedTask.from_dict(task_data)
        return None
...
class AIAgent:
    def __init__(self, api_key: str, role: AgentRole, temperature: float = 0.7):
        ...
        self.model = genai.GenerativeModel(
            model_name='models/gemini-2.5-flash',  # Updated model
            generation_config={
    ...
    async def _optimize_solution(self, user_query: str, context: str, previous_iteration: Dict) -> AgentResponse:
        ...
        ðŸ‘¤ ORIGINAL USER QUERY: {user_query}

        ðŸ¤– ORIGINAL SOLUTION:
        {original_solution}

        ðŸ” ANALYSIS FEEDBACK:
        {analysis}

        âŒ IDENTIFIED ERRORS:
        {errors}
...
class QualityMetrics:
    ...
    def calculate_objective_quality(
        ...
        try:
            ...
            # After calculating each metric, boost good scores
            if scores['completeness'] > 0.85:
                scores['completeness'] = min(0.98, scores['completeness'] * 1.10)
            if scores['accuracy'] > 0.90:
                scores['accuracy'] = min(0.98, scores['accuracy'] * 1.05)
...
class MultiAgentSystem:
    ...
    async def process_query(self, user_query: str, context: str = "",
                            ...
                            file_manager: Optional[EnhancedTextFileManager] = None) -> MultiAgentSession:
        session_id = f"session_{int(time.time())}"

        # NEW: Check cache first
        if file_manager and hasattr(file_manager, 'cache'):
            context_hash = hashlib.md5(context.encode()).hexdigest()
            cache_key = file_manager.cache.generate_cache_key(
                query=user_query,
                context_hash=context_hash,
                agent_version="v1.0",
                model_params={'model': 'gemini-2.0', 'temperature': 0.7}
            )

            cached_response = file_manager.cache.get(cache_key)
            if cached_response:
        ...
        if progress_callback:
            progress_callback("context", 0,
                              f"ðŸ“Š Routing: {complexity.upper()} - {pipeline['description']}")
        ...
        if progress_callback:
            progress_callback("context", 5, f"ðŸ“œ Using context ({len(context.split()) if context else 0} words)")
        ...
        while current_iteration < pipeline['max_iterations']:
            ...
            if progress_callback:
                progress_callback("generator", 20 + (current_iteration * 25),
                                  "ðŸ”§ Generator Agent creating solution...")
            ...
            if can_exit:
                logger.info(
                    f"âœ“ Quality threshold met: {quality_score:.3f} >= {current_threshold:.3f} "
                    f"(iteration {current_iteration + 1})"
                )
                break
            else:
                ...
                logger.info(
                    f"âš  Quality threshold not met: {quality_score:.3f} < {current_threshold:.3f} "
        ...
        if file_manager and hasattr(file_manager, 'cache'):
            file_manager.cache.set(cache_key, session, ttl_class='stable')
...
class AutonomousMultiAgentAssistant:
    def __init__(self, gemini_api_key: str):
        ...
        self.file_manager = EnhancedTextFileManager()  # âœ… NEW
    ...
    async def process_user_message(self, user_id: str, message: str, reset_context: bool = False,
        ...
        # Detect implementation/roadmap queries
        implementation_keywords = ['implement', 'roadmap', 'next steps', 'clarification',
                                   'build', 'start', 'how to', 'phase', 'day ', 'week ', 'tasks']
        force_full_docs = any(kw in message.lower() for kw in implementation_keywords)

        # Build context based on query type
        if force_full_docs:
            # Check cache first for full document analysis
            documents = self.file_manager.get_user_documents(user_id, 50)
            if documents:
                doc_cache_key = f"full_doc_{documents[0].id}_{message[:50]}"
                cached_response = self.file_manager.cache.get(doc_cache_key)

                if cached_response:
                    print(f"DEBUG: Using cached full-document response")
                    return cached_response

            # Use full document mode for technical queries
            context = self.file_manager.get_conversation_context_smart(
                user_id, 70, include_full_docs=True
            )
            print(f"DEBUG: Full document mode - {len(context.split())} words")
        else:
            # Semantic search for general queries
            conv_context = self.file_manager.get_conversation_context_smart(
                user_id, 70, include_full_docs=False
            )
            relevant_chunks = self.file_manager.get_relevant_document_chunks(
                message, user_id, top_k=20
            )
            context = "\n\n".join([conv_context, relevant_chunks] if relevant_chunks else [conv_context])
            print(f"DEBUG: Semantic mode - {len(context.split())} words")
        ...
        # Cache full-document responses
        if force_full_docs and documents:
            self.file_manager.cache.set(doc_cache_key, multi_agent_session.final_response, ttl_class='permanent')
    ...
    @log_errors
    async def process_user_message_with_context(self, user_id: str, message: str,
                                                doc_mode: str = 'summary',
                                                history_limit: int = 10,
                                                progress_callback: Optional[Callable] = None) -> str:
        """Process message with custom context configuration"""

        # Build context based on preferences
        if doc_mode == 'full':
            context = self.file_manager.get_conversation_context_smart(user_id, history_limit, include_full_docs=True)
        elif doc_mode == 'summary':
            context = self.file_manager.get_conversation_context_smart(user_id, history_limit, include_full_docs=False)
        else:  # 'none'
            # Only conversation history, no documents
            session_conversations = self.file_manager.get_session_conversations(user_id, history_limit // 2)
            historical_conversations = self.file_manager.get_recent_conversations(user_id, history_limit // 2)

            context_parts = ["=== ðŸ“œ CONVERSATION CONTEXT ==="]
            all_convs = session_conversations + historical_conversations
            all_convs.sort(key=lambda x: x.timestamp, reverse=True)

            for i, conv in enumerate(all_convs[:history_limit]):
                context_parts.append(f"\n--- Conversation {i + 1} ---")
                context_parts.append(f"ðŸ‘¤ USER: {conv.user_message}")
                context_parts.append(f"ðŸ¤– ASSISTANT: {conv.ai_response[:300]}...")

            context = "\n".join(context_parts)

        # Debug output
        print(f"DEBUG: Context mode={doc_mode}, history={history_limit}, size={len(context.split())} words")
...
class RichMultiAgentCLI:
    def __init__(self, api_key: str):
        ...
        # File manager is already EnhancedTextFileManager from assistant
    ...
    # Add this temporary method to RichMultiAgentCLI class:

    def reindex_existing_documents(self):
        """Re-generate embeddings for existing documents"""
        progress = self.create_progress()
        task = progress.add_task("[bold cyan]ðŸ”„ Re-indexing documents...[/bold cyan]", total=100)

        with Live(progress, auto_refresh=True, console=self.console):
            documents = self.assistant.file_manager.get_user_documents(self.user_id, 1000)

            for i, doc in enumerate(documents):
                progress.update(task, completed=int((i / len(documents)) * 100))

                # Generate embeddings
                result = self.assistant.file_manager.embedding_store.ingest_document(
                    doc, self.user_id
                )
                self.console.print(f"âœ… Indexed: {doc.filename} ({result['chunks']} chunks)")

            progress.update(task, completed=100)

        self.console.print("[bold green]âœ… Re-indexing complete![/bold green]")

    def display_embedding_stats(self):
        """Display embedding store statistics"""
        stats = self.assistant.file_manager.get_embedding_stats()

        stats_text = f"""
        [bold cyan]ðŸ“Š Embedding Store Statistics:[/bold cyan]

        â€¢ Version: [bold]{stats.get('version', 'N/A')}[/bold]
        â€¢ Total Chunks: [bold]{stats.get('total_chunks', 0)}[/bold]
        â€¢ Unique Documents: [bold]{stats.get('unique_documents', 0)}[/bold]
        â€¢ Unique Users: [bold]{stats.get('unique_users', 0)}[/bold]
        â€¢ Embedding Dimension: [bold]{stats.get('embedding_dim', 0)}[/bold]
        â€¢ Chunk Size: [bold]{stats.get('chunk_size', 0)} tokens[/bold]
        â€¢ Chunk Overlap: [bold]{stats.get('chunk_overlap', 0)} tokens[/bold]
        """

        self.console.print(Panel(stats_text, border_style="cyan", padding=(1, 2)))

    def display_cache_stats(self):
        """Display cache statistics"""
        stats = self.assistant.file_manager.get_cache_stats()

        hit_rate = stats['hit_rate'] * 100
        hit_color = "green" if hit_rate > 70 else "yellow" if hit_rate > 40 else "red"

        stats_text = f"""
        [bold cyan]ðŸ’¾ Cache Statistics:[/bold cyan]

        â€¢ Active Entries: [bold]{stats['active_entries']}[/bold]
        â€¢ Expired Entries: [bold]{stats['expired_entries']}[/bold]
        â€¢ Cache Hits: [bold green]{stats['hits']}[/bold green]
        â€¢ Cache Misses: [bold red]{stats['misses']}[/bold red]
        â€¢ Hit Rate: [bold {hit_color}]{hit_rate:.1f}%[/bold {hit_color}]
        â€¢ Cache Size: [bold]{stats['size_bytes'] / 1024:.1f} KB[/bold]

        [dim]Higher hit rate = fewer expensive AI calls = lower costs[/dim]
        """
    ...
    def clear_cache(self):
        ...
        if Confirm.ask("[bold yellow]Clear all cached responses?[/bold yellow]"):
            self.assistant.file_manager.cache.invalidate()
            self.console.print("[bold green]âœ… Cache cleared![/bold green]")

    def suggest_query_chunking(self, original_query: str):
        """Suggest breaking query into smaller parts"""
        suggestion = f"""
        [bold yellow]ðŸ’¡ Suggestion: Break Your Query Into Parts[/bold yellow]

        Your query: "{original_query[:100]}..."

        Try asking about ONE module at a time instead:

        â€¢ [bold cyan]"Solve module 4 from SE_2 using the uploaded PDF"[/bold cyan]
        â€¢ [bold cyan]"Now solve module 5 from SE_2"[/bold cyan]
        â€¢ [bold cyan]"Finally solve module 10 from SE_2"[/bold cyan]

        This helps avoid API limits and safety filters.
        """
        self.console.print(Panel(suggestion, border_style="yellow", padding=(1, 2)))

    def display_routing_stats(self):
        """Display intelligent routing statistics"""
        progress = self.create_progress()
        task = progress.add_task("[bold cyan]ðŸ“Š Loading routing statistics...[/bold cyan]", total=100)
        ...
        with Live(progress, auto_refresh=True, console=self.console, refresh_per_second=20):
            ...
            progress.update(task, completed=70,
                            description="[bold cyan]ðŸ“Š Analyzing routing efficiency...[/bold cyan]")
            ...
            progress.update(task, completed=100, description="[bold green]âœ… Statistics loaded![/bold green]")

        if "message" in stats:
            self.console.print(Panel(
                f"[yellow]{stats['message']}[/yellow]",
                title="[bold]ðŸ“Š Routing Statistics[/bold]",
                border_style="yellow"
            ))
            return
        ...
        self.console.print(Panel(
            routing_table,
            title="[bold cyan]ðŸ“Š Query Routing Breakdown[/bold cyan]",
            border_style="cyan",
            padding=(1, 1)
        ))
        ...
        # Efficiency summary
        efficiency_text = f"""
               [bold]ðŸŽ¯ Routing Efficiency:[/bold]
               â€¢ Total Queries Processed: [bold]{stats['total_queries']}[/bold]
               â€¢ Total API Calls Used: [bold]{stats['total_api_calls']}[/bold]
               â€¢ Average Calls per Query: [bold]{stats['avg_calls_per_query']}[/bold]
               â€¢ API Calls Saved: [bold green]{stats['api_calls_saved']}[/bold green]

               [bold yellow]ðŸ’¡ Cost Optimization:[/bold yellow]
               â€¢ Without routing: [dim]{stats['total_queries'] * 4} API calls[/dim]
               â€¢ With routing: [bold green]{stats['total_api_calls']} API calls[/bold green]
               â€¢ Efficiency gain: [bold green]{round((1 - stats['avg_calls_per_query'] / 4) * 100, 1)}%[/bold green]

               [bold cyan]ðŸ“ˆ Distribution:[/bold cyan]
               â€¢ Simple queries (fast): {stats['simple_queries']} ({round(stats['simple_queries'] / total * 100, 1) if total > 0 else 0}%)
               â€¢ Medium queries (balanced): {stats['medium_queries']} ({round(stats['medium_queries'] / total * 100, 1) if total > 0 else 0}%)
               â€¢ Complex queries (thorough): {stats['complex_queries']} ({round(stats['complex_queries'] / total * 100, 1) if total > 0 else 0}%)
               """
        ...
        if total > 0:
            simple_bar = "â–ˆ" * int(stats['simple_queries'] / total * 20)
            medium_bar = "â–ˆ" * int(stats['medium_queries'] / total * 20)
            complex_bar = "â–ˆ" * int(stats['complex_queries'] / total * 20)
    ...
    â•‘                                                                           â•‘
    â•‘  â–ˆâ–ˆâ•—     â–ˆâ–ˆâ•—  â–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ•—â–ˆâ–ˆâ•—   â–ˆâ–ˆâ•— â–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ•— â–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ•—  â–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ•— â–ˆâ–ˆâ•—  â–ˆâ–ˆâ•—         â•‘
    â•‘  â–ˆâ–ˆâ•‘    â–ˆâ–ˆâ–ˆâ–ˆâ•‘    â–ˆâ–ˆâ–ˆâ•”â•â•šâ–ˆâ–ˆâ•— â–ˆâ–ˆâ•”â•â–ˆâ–ˆâ•”â•â•â•â•â•â–ˆâ–ˆâ•”â•â•â•â–ˆâ–ˆâ•—â–ˆâ–ˆâ•”â•â•â•â–ˆâ–ˆâ•—â–ˆâ–ˆâ•‘ â–ˆâ–ˆâ•”â•         â•‘
    â•‘  â–ˆâ–ˆâ•‘   â–ˆâ–ˆâ•”â–ˆâ–ˆâ•‘   â–ˆâ–ˆâ–ˆâ•”â•  â•šâ–ˆâ–ˆâ–ˆâ–ˆâ•”â• â–ˆâ–ˆâ•‘     â–ˆâ–ˆâ•‘   â–ˆâ–ˆâ•‘â–ˆâ–ˆâ•‘   â–ˆâ–ˆâ•‘â–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ•”â•          â•‘
    â•‘  â–ˆâ–ˆâ•‘  â–ˆâ–ˆâ•”â•â–ˆâ–ˆâ•‘  â–ˆâ–ˆâ–ˆâ•”â•    â•šâ–ˆâ–ˆâ•”â•  â–ˆâ–ˆâ•‘     â–ˆâ–ˆâ•‘   â–ˆâ–ˆâ•‘â–ˆâ–ˆâ•‘   â–ˆâ–ˆâ•‘â–ˆâ–ˆâ•”â•â–ˆâ–ˆâ•—          â•‘
    â•‘  â–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ•”â• â–ˆâ–ˆâ•‘ â–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ•—   â–ˆâ–ˆâ•‘   â•šâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ•—â•šâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ•”â•â•šâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ•”â•â–ˆâ–ˆâ•‘  â–ˆâ–ˆâ•—         â•‘
    â•‘  â•šâ•â•â•â•â•â•  â•šâ•â• â•šâ•â•â•â•â•â•â•   â•šâ•â•    â•šâ•â•â•â•â•â• â•šâ•â•â•â•â•â•  â•šâ•â•â•â•â•â• â•šâ•â•  â•šâ•â•         â•‘
    â•‘                                                                           â•‘
    â•‘                     [dim]ðŸ”¥ === Let it cook! === ðŸ”¥[/dim]                  â•‘
    â•‘                                                                           â•‘
    ...
    def display_help(self):
        help_text = """
        [bold magenta]ðŸ“‹ Available Commands:[/bold magenta]
        â€¢ [bold green]chat[/bold green]         Start a conversation (type [bold red]back[/bold red] to exit)
        â€¢ [bold green]embeddings[/bold green]   View embedding store statistics (NEW!)
        â€¢ [bold green]cache[/bold green]        View cache statistics (NEW!)
        â€¢ [bold green]clearcache[/bold green]   Clear response cache (NEW!)
        â€¢ [bold green]download[/bold green]     ðŸ’¾ Download chat history (NEW!)
        â€¢ [bold green]insights[/bold green]     View user interaction statistics
        â€¢ [bold green]docs[/bold green]         Manage uploaded documents (view/upload)
        â€¢ [bold green]maintenance[/bold green]  ðŸ”§ Cleanup & management
        â€¢ [bold green]tasks[/bold green]        Show autonomous tasks and status
        â€¢ [bold green]quality[/bold green]      Display recent quality scores
        â€¢ [bold green]routing[/bold green]      ðŸ“Š View intelligent routing statistics (NEW!)
        â€¢ [bold green]agents[/bold green]       Show multi-agent architecture
        â€¢ [bold green]context[/bold green]      Preview conversation context
        â€¢ [bold green]files[/bold green]        Check data file status
        â€¢ [bold green]stats[/bold green]        System performance statistics
        â€¢ [bold green]cls[/bold green]          Clear the console screen
        â€¢ [bold green]help[/bold green]         Show this help menu
        â€¢ [bold red]quit[/bold red]            Exit the application

        [bold yellow]ðŸ”§ Maintenance Features:[/bold yellow]
        â€¢ Delete specific documents or clear all documents
        â€¢ Clear conversations with various options (all/recent)
        â€¢ Document capacity management (100 docs / 100MB limit)
        â€¢ Full system cleanup options

        [bold cyan]âš¡ Shortcuts:[/bold cyan]
        â€¢ Type 'r' for routing stats
        â€¢ Type 'c' for chat mode
        â€¢ Type 'i' for insights
        â€¢ Type 'q' to quit

        [dim]Type 'back' to return to main menu from any command.[/dim]
        [bold yellow]âš ï¸ Note:[/bold yellow] The system uses conversation history and uploaded documents
    ...
    def display_agents(self):
        ...
        agents_info = """
        [bold cyan]ðŸ¤– Multi-Agent Architecture:[/bold cyan]
        â€¢ [bold green]Generator Agent[/bold green]: Creates initial solutions (temp: 0.8)
        â€¢ [bold yellow]Analyzer Agent[/bold yellow]: Reviews for errors (temp: 0.3)  
        â€¢ [bold blue]Optimizer Agent[/bold blue]: Refines solutions (temp: 0.5)
        â€¢ [bold cyan]Validator Agent[/bold cyan]: Validates accuracy (temp: 0.2)
        """
        self.console.print(Panel(agents_info, border_style="cyan"))

    def get_context_preference(self) -> tuple[bool, int]:
        """Interactive context configuration before each message"""
        context_menu = """
        [bold cyan]ðŸ“‹ Context Configuration:[/bold cyan]

        [bold yellow]Document Inclusion:[/bold yellow]
        â€¢ [bold green]1[/bold green] - Full documents (high token usage, detailed answers)
        â€¢ [bold blue]2[/bold blue] - Document summaries only (low token usage, faster)
        â€¢ [bold dim]3[/bold dim] - No documents (minimal tokens)

        [bold yellow]Conversation History:[/bold yellow]
        â€¢ [bold green]a[/bold green] - Last 5 conversations
        â€¢ [bold blue]b[/bold blue] - Last 10 conversations  
        â€¢ [bold yellow]c[/bold yellow] - Last 20 conversations
        â€¢ [bold red]d[/bold red] - Last 50 conversations
        â€¢ [bold dim]e[/bold dim] - No history

        [dim]Press Enter for defaults (summaries + 10 conversations)[/dim]
        """

        self.console.print(Panel(context_menu, border_style="cyan", padding=(1, 2)))

        # Get document preference
        doc_choice = Prompt.ask(
            "[bold cyan]Document mode (1-3)[/bold cyan]",
            default="2"
        ).strip()
        ...
        doc_mode = doc_mode_map.get(doc_choice, 'summary')

        # Get history preference
        history_choice = Prompt.ask(
            "[bold cyan]History amount (a-e)[/bold cyan]",
            default="b"
        ).strip().lower()
        ...
        history_limit = history_map.get(history_choice, 10)

        # Show selection summary
        summary = f"""
        [bold green]âœ“ Context configured:[/bold green]
        â€¢ Documents: {doc_mode.upper()}
        â€¢ History: {history_limit} conversations
        """
        self.console.print(Panel(summary, border_style="green", padding=(0, 2)))

        return (doc_mode, history_limit)

    def display_tasks(self):
        """Display autonomous tasks and their status"""
        progress = self.create_progress()
        task = progress.add_task("[bold cyan]âš™ï¸ Loading tasks...[/bold cyan]", total=100)
        ...
        with Live(progress, auto_refresh=True, console=self.console, refresh_per_second=20):
            ...
            progress.update(task, completed=60, description="[bold cyan]âš™ï¸ Analyzing task queue...[/bold cyan]")
            ...
            for task_data in recent_tasks:
                status_color = {
                    ...
                    'in_progress': '[blue]ðŸ”„ Running[/blue]',
                    'completed': '[green]âœ… Done[/green]',
                    'failed': '[red]âŒ Failed[/red]'
                }.get(task_data.get('status', 'unknown'), '[dim]â“ Unknown[/dim]')

                priority = task_data.get('priority', 0)
                priority_display = f"ðŸ”¥ {priority}" if priority >= 3 else f"ðŸ“Œ {priority}" if priority >= 2 else f"ðŸ“‹ {priority}"
            ...
            progress.update(task, completed=100, description="[bold green]âœ“ Tasks loaded![/bold green]")

        # Display results
        self.console.print(Panel(
            tasks_table,
            title="[bold cyan]âš™ï¸ Autonomous Tasks[/bold cyan]",
            border_style="cyan",
            padding=(1, 1)
        ))
        ...
        â€¢ Total Tasks: [bold]{len(all_tasks_data)}[/bold]
        â€¢ Pending: [bold yellow]{status_counts.get('pending', 0)}[/bold yellow]
        â€¢ Completed: [bold green]{status_counts.get('completed', 0)}[/bold green]
        â€¢ Failed: [bold red]{status_counts.get('failed', 0)}[/bold red]
        â€¢ Active Queue: [bold]{len(pending_tasks)} ready to execute[/bold]
        """
    ...
    def display_quality(self):
        ...
        progress = self.create_progress()
        task = progress.add_task("[bold cyan]ðŸ“ˆ Analyzing quality...[/bold cyan]", total=100)
        ...
        with Live(progress, auto_refresh=True, console=self.console, refresh_per_second=20):
            ...
            progress.update(task, completed=70, description="[bold cyan]ðŸ“ˆ Computing quality metrics...[/bold cyan]")
            ...
            progress.update(task, completed=100, description="[bold green]âœ“ Quality analysis complete![/bold green]")

        if not quality_sessions:
            self.console.print(Panel(
                "[yellow]No multi-agent sessions found for quality analysis[/yellow]",
                title="[bold]ðŸ“ˆ Quality Metrics[/bold]",
                border_style="yellow"
            ))
            return
        ...
        for session in quality_sessions:
            ...
            if score >= 0.85:
                quality_display = f"[bold green]{score:.2f}[/bold green] ðŸ”¥"
            elif score >= 0.70:
                quality_display = f"[bold yellow]{score:.2f}[/bold yellow] ðŸ“ˆ"
            else:
                quality_display = f"[bold red]{score:.2f}[/bold red] âš ï¸"
        ...
        self.console.print(Panel(
            quality_table,
            title="[bold cyan]ðŸ“ˆ Quality Metrics[/bold cyan]",
            border_style="cyan",
            padding=(1, 1)
        ))
        ...
        if quality_sessions:
            ...
            quality_trend = "ðŸ“ˆ Improving" if len(scores) >= 3 and scores[0] > scores[-1] else "ðŸ“Š Stable" if len(
                scores) >= 3 else "ðŸ” Analyzing"

            stats_text = f"""
            [bold]ðŸ“Š Quality Statistics:[/bold]
            â€¢ Average Quality: [bold]{avg_quality:.2f}[/bold] ({quality_trend})
            â€¢ Highest Score: [bold green]{max_quality:.2f}[/bold green]
            â€¢ Lowest Score: [bold red]{min_quality:.2f}[/bold red]
            â€¢ Avg Iterations: [bold]{avg_iterations:.1f}[/bold]
            â€¢ Sessions Analyzed: [bold]{len(quality_sessions)}[/bold]
            â€¢ Quality Threshold: [bold]0.85[/bold] (system target)
            """
    ...
    def display_files(self):
        ...
        progress = self.create_progress()
        task = progress.add_task("[bold cyan]ðŸ“ Checking file status...[/bold cyan]", total=100)
        ...
        with Live(progress, auto_refresh=True, console=self.console, refresh_per_second=20):
            ...
            progress.update(task, completed=70, description="[bold cyan]ðŸ“ Analyzing file system...[/bold cyan]")
            ...
            for file_name, exists in stats['files_exist'].items():
                status = "[green]âœ“ Active[/green]" if exists else "[red]âœ— Missing[/red]"
                record_count = "0"
            ...
            progress.update(task, completed=100, description="[bold green]âœ“ File status loaded![/bold green]")

        # Display results
        self.console.print(Panel(
            files_table,
            title="[bold cyan]ðŸ“ Data File Status[/bold cyan]",
            border_style="cyan",
            padding=(1, 1)
        ))
        ...
        info_text = f"""
        [bold]ðŸ“Š Storage Summary:[/bold]
        â€¢ Data Directory: [bold]./multi_agent_data/[/bold]
        â€¢ Total Users: [bold]{len(stats['users'])}[/bold]
        â€¢ Oldest Record: [bold]{stats['oldest_conversation'][:10] if stats['oldest_conversation'] != 'none' else 'None'}[/bold]
        â€¢ Newest Record: [bold]{stats['newest_conversation'][:10] if stats['newest_conversation'] != 'none' else 'None'}[/bold]
        """
    ...
    def display_stats(self):
        ...
        progress = self.create_progress()
        task = progress.add_task("[bold cyan]ðŸ“Š Computing statistics...[/bold cyan]", total=100)
        ...
        with Live(progress, auto_refresh=True, console=self.console, refresh_per_second=20):
            ...
            progress.update(task, completed=60, description="[bold cyan]ðŸ“Š Analyzing performance...[/bold cyan]")
            ...
            progress.update(task, completed=100, description="[bold green]âœ“ Statistics computed![/bold green]")

        # Display results
        self.console.print(Panel(
            stats_table,
            title="[bold cyan]ðŸ“Š System Performance Statistics[/bold cyan]",
            border_style="cyan",
            padding=(1, 1)
        ))
        ...
        perf_text = f"""
        [bold]ðŸŽ¯ Performance Indicators:[/bold]
        â€¢ Quality Trend: [bold]{"ðŸ”¥ Excellent" if user_insights.get('average_quality_score', 0) > 0.8 else "ðŸ“ˆ Good" if user_insights.get('average_quality_score', 0) > 0.6 else "âš ï¸ Improving"}[/bold]
        â€¢ Processing Efficiency: [bold]{"âš¡ Fast" if user_insights.get('average_iterations', 0) < 2 else "ðŸ”„ Standard" if user_insights.get('average_iterations', 0) < 3 else "ðŸŒ Complex"}[/bold]
        â€¢ Memory Usage: [bold]{"ðŸ’¾ {:.1f}MB".format(sum([len(str(storage_stats)) for _ in range(storage_stats['total_conversations'])]) / 1024)}[/bold]
        â€¢ Context Utilization: [bold]{"ðŸŽ¯ High" if user_insights.get('context_usage_rate', 0) > 70 else "ðŸ“Š Medium" if user_insights.get('context_usage_rate', 0) > 30 else "ðŸ“‰ Low"}[/bold]
        """
    ...
    def display_response(self, user_message: str, ai_response: str, processing_time: float, context_used: str = ""):
        ...
        user_panel = Panel(
            Text(f"ðŸ‘¤ You", style="bold blue") + Text(f": {user_message}", style="italic blue"),
            title="Your Message",
        ...
        try:
            ...
            cleaned_response = ai_response.replace("\n\n", "\n").replace("# ",
                                                                         "ðŸ”¹ ")  # Headers to icons, clean extra newlines
            cleaned_response = cleaned_response.replace("* ", "â€¢ ").replace("- ", "â€¢ ")  # Bullets without raw *
            cleaned_response = "\n".join(
        ...
        if topics:
            topics_bullets = "\n".join(
                [f"â€¢ [bold cyan]{topic}[/bold cyan]" for topic in topics[:5]])  # Limit to 5, bold
            topics_content = Panel(
                Text(topics_bullets, style="white"),
                title=Text("ðŸ·ï¸ Key Topics", style="bold magenta"),
    ...
    def display_quality_breakdown(self, quality_metrics: Dict[str, Any]):
        ...
        self.console.print(Panel(
            quality_table,
            title="[bold cyan]ðŸ“Š Quality Analysis[/bold cyan]",
            border_style="cyan",
            padding=(1, 1)
        ))
        ...
        â€¢ Objective Metrics: [bold cyan]{objective['overall']:.3f}[/bold cyan] (40% weight)
        â€¢ Agent Confidence: [bold yellow]{quality_metrics.get('subjective', 0):.3f}[/bold yellow] (60% weight)
        â€¢ Combined Score: [bold green]{quality_metrics.get('combined', 0):.3f}[/bold green]
        â€¢ Quality Tier: [bold]{quality_metrics.get('tier', 'Unknown')}[/bold]
        """
    ...
    def show_agent_progress(self, message: str):
        ...
        with Live(auto_refresh=True, console=self.console, refresh_per_second=20) as live:
            ...
            # Main task
            main_task = progress.add_task("[bold cyan]ðŸ”„ Processing your request...[/bold cyan]", total=100)

            # Agent processing stages with animations
            stages = [
                ("context", 5, "[bold yellow]ðŸ“œ Analyzing context and documents...[/bold yellow]", "yellow"),
                ("generator", 30, "[bold green]ðŸ”§ Generator Agent creating solution...[/bold green]", "green"),
                ("analyzer", 50, "[bold yellow]ðŸ” Analyzer Agent reviewing for errors...[/bold yellow]", "yellow"),
                ("optimizer", 70, "[bold blue]âš¡ Optimizer Agent refining solution...[/bold blue]", "blue"),
                ("validator", 90, "[bold cyan]âœ… Validator Agent validating accuracy...[/bold cyan]", "cyan"),
                ("complete", 100, "[bold green]âœ“ All agents completed processing![/bold green]", "green")
            ]
    ...
    async def process_message_with_progress(self, message: str, doc_mode: str = 'summary',
        ...
        with Live(progress, auto_refresh=True, console=self.console, refresh_per_second=20):
            ...
            agent_stages = {
                "context": (5, "[bold yellow]ðŸ“œ Analyzing context...[/bold yellow]", "yellow"),
                "generator": (30, "[bold green]ðŸ”§ Generator Agent working...[/bold green]", "green"),
                "analyzer": (50, "[bold yellow]ðŸ” Analyzer Agent reviewing...[/bold yellow]", "yellow"),
                "optimizer": (70, "[bold blue]âš¡ Optimizer Agent refining...[/bold blue]", "blue"),
                "validator": (90, "[bold cyan]âœ… Validator Agent validating...[/bold cyan]", "cyan")
            }
            ...
            response = await self.assistant.process_user_message_with_context(
                self.user_id,
                message,
                doc_mode=doc_mode,
            ...
            progress.update(main_task, completed=100, description="[bold green]âœ“ Processing complete![/bold green]")
    ...
    def display_insights(self):
        ...
        progress = self.create_progress()
        task = progress.add_task("[bold cyan]ðŸ“Š Loading insights...[/bold cyan]", total=100)
        ...
        with Live(progress, auto_refresh=True, console=self.console, refresh_per_second=20):
            ...
            progress.update(task, completed=60, description="[bold cyan]ðŸ“Š Analyzing conversation data...[/bold cyan]")
            # Animate to 80%
            current_percent = 60
            while current_percent < 80:
                current_percent += 1
            ...
            if "message" in insights:
                self.console.print(Panel(f"[yellow]{insights['message']}[/yellow]", title="[bold]ðŸ“Š Insights[/bold]",
                                         border_style="yellow"))
            else:
                stats_text = f"""
                [bold]ðŸ“ˆ Session Statistics:[/bold]
                â€¢ Total Conversations: [bold]{insights.get('total_conversations', 0)}[/bold]
                â€¢ Multi-Agent Sessions: [bold]{insights.get('multi_agent_sessions', 0)}[/bold]
                â€¢ Avg. Quality Score: [bold]{insights.get('average_quality_score', 0):.2f}[/bold]
                â€¢ Avg. Iterations: [bold]{insights.get('average_iterations', 0):.1f}[/bold]
                â€¢ Context Usage Rate: [bold]{insights.get('context_usage_rate', 0)}%[/bold]
                â€¢ Session Duration: [bold]{datetime.now() - self.session_start}[/bold]
                """
                self.console.print(
                    Panel(stats_text, title="[bold cyan]ðŸ“Š Insights[/bold cyan]", border_style="cyan", padding=(1, 2)))

                if insights.get('topics'):
                    topics_text = "\n".join(
                        [f"â€¢ [bold]{topic}[/bold]: {count}" for topic, count in insights['topics'][:5]])
                    self.console.print(Panel(
                        f"[bold magenta]ðŸ·ï¸ Topics:[/bold magenta]\n{topics_text}",
                        title="[bold magenta]ðŸ·ï¸ Discussion Topics[/bold magenta]",
            ...
            progress.update(task, completed=100, description="[bold green]âœ“ Insights loaded![/bold green]")

    def display_context(self):
        """Display context preview with animated progress"""
        progress = self.create_progress()
        task = progress.add_task("[bold cyan]ðŸ“œ Loading context...[/bold cyan]", total=100)
        ...
        with Live(progress, auto_refresh=True, console=self.console, refresh_per_second=20):
            ...
            progress.update(task, completed=70, description="[bold cyan]ðŸ“œ Compiling context preview...[/bold cyan]")
            ...
            if not context or context == "No previous conversation history available.":
                self.console.print(Panel(
                    ...
                    title="[bold]ðŸ“„ Context Preview[/bold]",
                    border_style="yellow"
                ))
            else:
                ...
                context_panel = Panel(
                    preview_text,
                    title=f"[bold cyan]ðŸ“„ Context Preview ({len(context.split())} words)[/bold cyan]",
                    border_style="cyan",
                    padding=(1, 2)
                )
                self.console.print(context_panel)
                ...
                â€¢ Total words: {len(context.split())}
                â€¢ Conversations: {len([line for line in context_lines if line.startswith('--- Conversation')])}
                â€¢ Documents referenced: {len([line for line in context_lines if line.startswith('--- Document')])}
                """
                self.console.print(
                    Panel(stats, title="[bold]ðŸ“Š Context Stats[/bold]", border_style="blue", padding=(1, 1)))
            ...
            progress.update(task, completed=100, description="[bold green]âœ“ Context loaded![/bold green]")
    ...
    def upload_document(self):
        ...
        if capacity_info['at_document_limit']:
            self.console.print(Panel(
                f"[red]âŒ Document limit reached![/red]\n"
        ...
        if capacity_info['at_size_limit']:
            self.console.print(Panel(
                f"[red]âŒ Storage limit reached![/red]\n"
        ...
        file_path = Prompt.ask("[bold cyan]ðŸ“ Enter file path to upload[/bold cyan] (or 'back' to cancel)")
        ...
        if file_size > 50 * 1024 * 1024:
            self.console.print("[red]File too large. Maximum size is 50MB per file[/red]")
            return None

        progress = self.create_progress()
        task = progress.add_task("[bold cyan]ðŸ“ Preparing upload...[/bold cyan]", total=100)
        ...
        with Live(progress, auto_refresh=True, console=self.console, refresh_per_second=20):
            ...
            while current_percent < 20:
                current_percent += 2
                progress.update(task, completed=current_percent,
                                description=f"[bold cyan]ðŸ“ Analyzing file ({decimal(file_size)})...[/bold cyan]")
                time.sleep(0.05)

            # Simulate processing
            progress.update(task, completed=60, description="[bold cyan]ðŸ’¾ Uploading and indexing...[/bold cyan]")
            current_percent = 60
            try:
                ...
                if document:
                    progress.update(task, completed=100,
                                    description="[bold green]âœ… Document uploaded successfully![/bold green]")
                    ...
                    self.console.print(
                        Panel(info_table, title="[bold green]ðŸ“„ Document Info[/bold green]", border_style="green",
                              padding=(1, 2)))
                else:
                    progress.update(task, completed=100,
                                    description="[bold red]âŒ Failed to process document[/bold red]")
                    self.console.print("[red]Failed to process document[/red]")

                return document
            except Exception as e:
                progress.update(task, completed=100, description=f"[bold red]âŒ Error: {str(e)}[/bold red]")
    ...
    def display_documents(self):
        ...
        progress = self.create_progress()
        task = progress.add_task("[bold cyan]ðŸ“ Loading documents...[/bold cyan]", total=100)
        ...
        with Live(progress, auto_refresh=True, console=self.console, refresh_per_second=20):
            ...
            progress.update(task, completed=60, description="[bold cyan]ðŸ“ Compiling document list...[/bold cyan]")
            ...
            if not documents:
                self.console.print(
                    Panel("[yellow]No documents uploaded yet[/yellow]", title="[bold]ðŸ“„ Your Documents[/bold]",
                          border_style="yellow"))
            else:
                docs_table = Table(
                    show_header=True,
                ...
                self.console.print(Panel(
                    docs_table,
                    title=f"[bold cyan]ðŸ“„ Your Documents ({len(documents)} total)[/bold cyan]",
                    border_style="cyan",
                    padding=(1, 1)
                ))
            ...
            progress.update(task, completed=100, description="[bold green]âœ“ Documents loaded![/bold green]")
    ...
    def delete_specific_document(self):
        ...
        if not documents:
            self.console.print(Panel(
                ...
                title="[bold]ðŸ“„ Delete Document[/bold]",
                border_style="yellow"
            ))
            return
        ...
        self.console.print(Panel(
            docs_table,
            title="[bold red]ðŸ—‘ï¸ Delete Document[/bold red]",
            border_style="red",
            padding=(1, 1)
        ))
            ...
            if 0 <= doc_index < len(documents):
                ...
                if Confirm.ask(f"[bold red]Are you sure you want to delete '{selected_doc.filename}'?[/bold red]"):
                    if self.assistant.file_manager.delete_document(selected_doc.id, self.user_id):
                        self.console.print(
                            f"[bold green]âœ… Document '{selected_doc.filename}' deleted successfully![/bold green]")
                    else:
                        self.console.print(
                            f"[bold red]âŒ Failed to delete document '{selected_doc.filename}'[/bold red]")
            else:
    ...
    def clear_conversations_menu(self):
        ...
        options_text = """
        [bold magenta]ðŸ—‘ï¸ Clear Conversations Options:[/bold magenta]

        â€¢ [bold green]1[/bold green] - Clear ALL conversations (complete reset)
        â€¢ [bold yellow]2[/bold yellow] - Clear old conversations (keep recent 20)
        â€¢ [bold blue]3[/bold blue] - Clear old conversations (keep recent 10)
        â€¢ [bold red]4[/bold red] - Clear old conversations (keep recent 5)
        â€¢ [bold dim]back[/bold dim] - Return to main menu

        [bold yellow]âš ï¸ Warning:[/bold yellow] This action cannot be undone!
        """
        ...
        if choice == '1':
            ...
            if Confirm.ask(
                ...
                progress = self.create_progress()
                task = progress.add_task("[bold red]ðŸ—‘ï¸ Clearing all conversations...[/bold red]", total=100)
                ...
                if success:
                    self.console.print("[bold green]âœ… All conversations cleared successfully![/bold green]")
                    # Clear cached context
                    self.assistant.clear_cached_context(self.user_id)
                else:
                    self.console.print("[bold red]âŒ Failed to clear conversations[/bold red]")
            ...
            if Confirm.ask(
                ...
                progress = self.create_progress()
                task = progress.add_task(f"[bold yellow]ðŸ—‘ï¸ Clearing {to_delete} old conversations...[/bold yellow]",
                                         total=100)
                ...
                self.console.print(
                    f"[bold green]âœ… Cleared {deleted_count} old conversations, kept {keep_recent} recent ones![/bold green]")
    ...
    def manage_documents_menu(self):
        ...
        while True:
            ...
            usage_bar = "â–ˆ" * int(capacity_info['usage_percentage'] / 5) + "â–‘" * (
            ...
            menu_text = f"""
            [bold magenta]ðŸ“„ Document Management:[/bold magenta]

            [bold]Storage Usage:[/bold]
            [{capacity_color}]{usage_bar}[/{capacity_color}] {capacity_info['usage_percentage']}%
            â€¢ Documents: {capacity_info['total_documents']}/{capacity_info['max_documents']} ({capacity_info['documents_remaining']} remaining)
            â€¢ Storage: {capacity_info['total_size_mb']}MB/{capacity_info['max_size_mb']}MB ({capacity_info['size_remaining_mb']}MB remaining)

            [bold]Options:[/bold]
            â€¢ [bold green]1[/bold green] - View all documents
            â€¢ [bold blue]2[/bold blue] - Upload new document
            â€¢ [bold yellow]3[/bold yellow] - Delete specific document
            â€¢ [bold red]4[/bold red] - Clear all documents
            â€¢ [bold dim]back[/bold dim] - Return to main menu
            """

            # Add warnings if at limits
            if capacity_info['at_document_limit']:
                menu_text += "\n[bold red]âš ï¸ Document limit reached! Delete some documents to upload new ones.[/bold red]"
            if capacity_info['at_size_limit']:
                menu_text += "\n[bold red]âš ï¸ Storage limit reached! Delete some documents to free space.[/bold red]"
            ...
            if choice in ['back', 'cancel', 'exit']:
                ...
                if capacity_info['at_document_limit']:
                    self.console.print("[red]âŒ Document limit reached! Delete some documents first.[/red]")
                elif capacity_info['at_size_limit']:
                    self.console.print("[red]âŒ Storage limit reached! Delete some documents first.[/red]")
                else:
                ...
                if Confirm.ask(
                    ...
                    progress = self.create_progress()
                    task = progress.add_task("[bold red]ðŸ—‘ï¸ Clearing all documents...[/bold red]", total=100)
                    ...
                    self.console.print(
                        f"[bold green]âœ… Cleared {deleted_count} documents successfully![/bold green]")
                Prompt.ask("[bold]Press Enter to continue...[/bold]")
            else:
                self.console.print("[red]Invalid option selected[/red]")
    ...
    def display_maintenance_menu(self):
        ...
        maintenance_text = """
        [bold magenta]ðŸ”§ Maintenance & Cleanup:[/bold magenta]

        â€¢ [bold green]1[/bold green] - Clear conversations (with options)
        â€¢ [bold blue]2[/bold blue] - Manage documents (view/delete/capacity)
        â€¢ [bold yellow]3[/bold yellow] - View storage statistics
        â€¢ [bold red]4[/bold red] - Full system cleanup (conversations + documents)
        â€¢ [bold dim]back[/bold dim] - Return to main menu

        [bold yellow]âš ï¸ Warning:[/bold yellow] Cleanup operations cannot be undone!
        """
        ...
        if choice in ['back', 'cancel', 'exit']:
            ...
            warning_text = f"""
            [bold red]âš ï¸ FULL SYSTEM CLEANUP WARNING âš ï¸[/bold red]

            This will delete:
            â€¢ All {len(conversations)} conversations
            â€¢ All {len(documents)} documents
            â€¢ All cached context data
            ...
            if Confirm.ask("[bold red]Are you absolutely sure you want to perform a full cleanup?[/bold red]"):
                if Confirm.ask("[bold red]This is your final warning. Continue with full cleanup?[/bold red]"):
                    progress = self.create_progress()
                    task = progress.add_task("[bold red]ðŸ—‘ï¸ Performing full system cleanup...[/bold red]", total=100)

                    with Live(progress, auto_refresh=True, console=self.console, refresh_per_second=20):
                        # Clear conversations
                        progress.update(task, completed=25,
                                        description="[bold red]ðŸ—‘ï¸ Clearing conversations...[/bold red]")
                        conv_success = self.assistant.file_manager.clear_all_conversations(self.user_id)

                        # Clear documents
                        progress.update(task, completed=50,
                                        description="[bold red]ðŸ—‘ï¸ Clearing documents...[/bold red]")
                        doc_count = self.assistant.file_manager.clear_all_documents(self.user_id)

                        # Clear cache
                        progress.update(task, completed=75, description="[bold red]ðŸ—‘ï¸ Clearing cache...[/bold red]")
                        self.assistant.clear_cached_context(self.user_id)

                        # Complete
                        progress.update(task, completed=100,
                                        description="[bold green]âœ… Full cleanup complete![/bold green]")
                        time.sleep(1)

                    self.console.print(f"[bold green]âœ… Full cleanup completed successfully![/bold green]")
                    self.console.print(f"â€¢ Conversations cleared: {len(conversations)}")
                    self.console.print(f"â€¢ Documents cleared: {doc_count}")
                    self.console.print("â€¢ Cache cleared")
        else:
            self.console.print("[red]Invalid option selected[/red]")
    ...
    def download_chat_menu(self):
        ...
        menu_text = """
        [bold magenta]ðŸ’¾ Download Chat History:[/bold magenta]

        â€¢ [bold green]1[/bold green] - Save current chat only
        â€¢ [bold blue]2[/bold blue] - Select specific chats to download
        â€¢ [bold yellow]3[/bold yellow] - Download all chats (bulk export)
        â€¢ [bold cyan]4[/bold cyan] - Download last N chats (10/20/50)
        â€¢ [bold dim]back[/bold dim] - Cancel
        """
    ...
    def _bulk_download_all(self):
        ...
        progress = self.create_progress()
        task = progress.add_task("[bold cyan]ðŸ’¾ Exporting all chats...[/bold cyan]", total=100)
        ...
        if filepath:
            ...
            self.console.print(Panel(
                f"[bold green]âœ… Exported all chats![/bold green]\n"
                f"ðŸ“ {Path(filepath).name}\n"
                f"ðŸ“ {decimal(file_size)}",
                border_style="green"
            ))
    ...
    def _bulk_download_limited(self):
        ...
        progress = self.create_progress()
        task = progress.add_task(f"[bold cyan]ðŸ’¾ Exporting last {limit} chats...[/bold cyan]", total=100)
        ...
        if filepath:
            file_size = Path(filepath).stat().st_size
            self.console.print(Panel(
                f"[bold green]âœ… Exported last {limit} chats![/bold green]\n"
                f"ðŸ“ {Path(filepath).name}\n"
                f"ðŸ“ {decimal(file_size)}",
                border_style="green"
            ))
    ...
    def download_current_chat(self):
        ...
        menu_text = """
        [bold magenta]ðŸ’¾ Download Current Chat:[/bold magenta]

        Choose format:
        â€¢ [bold green]1[/bold green] - Plain Text (.txt)
        â€¢ [bold blue]2[/bold blue] - Markdown (.md)
        â€¢ [bold yellow]3[/bold yellow] - JSON (.json)
        â€¢ [bold dim]back[/bold dim] - Cancel
        """
        ...
        progress = self.create_progress()
        task = progress.add_task("[bold cyan]ðŸ’¾ Saving current chat...[/bold cyan]", total=100)
        ...
        with Live(progress, auto_refresh=True, console=self.console, refresh_per_second=20):
            ...
            progress.update(task, completed=100, description="[bold green]âœ… Current chat saved![/bold green]")

        if filepath:
            file_size = Path(filepath).stat().st_size

            success_text = f"""
            [bold green]âœ… Current chat saved successfully![/bold green]

            ðŸ“ File: [bold cyan]{Path(filepath).name}[/bold cyan]
            ðŸ“ Size: [bold]{decimal(file_size)}[/bold]
            ðŸ“ Location: [dim]{filepath}[/dim]
            """

            self.console.print(Panel(success_text, border_style="green", padding=(1, 2)))
        else:
            self.console.print("[red]âŒ No conversation to save[/red]")
    ...
    def download_selected_chats(self):
        ...
        # Load conversations
        progress = self.create_progress()
        task = progress.add_task("[bold cyan]ðŸ“‹ Loading conversations...[/bold cyan]", total=100)
        ...
        with Live(progress, auto_refresh=True, console=self.console, refresh_per_second=20):
            ...
            progress.update(task, completed=100, description="[bold green]âœ… Loaded![/bold green]")
        ...
        â€¢ Single: [bold]5[/bold] (conversation #5)
        â€¢ Range: [bold]1-10[/bold] (conversations 1 to 10)
        â€¢ Multiple: [bold]1,5,8[/bold] (conversations 1, 5, and 8)
        â€¢ All: [bold]all[/bold]
        â€¢ Cancel: [bold]back[/bold]
        """
        ...
        progress = self.create_progress()
        task = progress.add_task("[bold cyan]ðŸ’¾ Exporting selected chats...[/bold cyan]", total=100)
        ...
        with Live(progress, auto_refresh=True, console=self.console, refresh_per_second=20):
            ...
            progress.update(task, completed=100, description="[bold green]âœ… Export complete![/bold green]")

        file_size = filepath.stat().st_size

        success_text = f"""
        [bold green]âœ… Selected chats exported successfully![/bold green]

        ðŸ“Š Exported: [bold]{len(selected_convs)} conversation(s)[/bold]
        ðŸ“ File: [bold cyan]{filename}[/bold cyan]
        ðŸ“ Size: [bold]{decimal(file_size)}[/bold]
        ðŸ“ Location: [dim]{filepath}[/dim]
        """
    ...
    async def handle_chat_mode(self):
        ...
        self.console.print(Panel(
            "[bold cyan]ðŸ’¬ Entering chat mode. Type [bold red]back[/bold red] or [bold red]exit[/bold red] to return to main menu.[/bold cyan]\n"
            "[bold yellow]âš ï¸ Note:[/bold yellow] You can configure context before each message.",
            border_style="cyan",
            padding=(1, 2)
        ))

        while True:
            message = Prompt.ask("[bold cyan]ðŸ‘¤ Your message[/bold cyan]")
            if message.strip().lower() in ['back', 'exit']:
                break

            if message.strip():
                # ðŸ”§ FIX: Let user choose document mode
                self.console.print("\n[bold yellow]ðŸ“„ Document Mode:[/bold yellow]")
                self.console.print(
                    "  â€¢ [bold green]f[/bold green] - Full documents (for roadmaps, implementation plans)")
                self.console.print("  â€¢ [bold blue]s[/bold blue] - Semantic search (for Q&A, lookups)")
                self.console.print("  â€¢ [bold dim]n[/bold dim] - No documents")

                doc_choice = Prompt.ask(
                    "[bold cyan]Document mode[/bold cyan]",
                    default="f"  # ðŸ”§ Default to FULL for roadmaps
                ).strip().lower()

                doc_mode_map = {'f': 'full', 's': 'summary', 'n': 'none'}
                doc_mode = doc_mode_map.get(doc_choice, 'full')

                # Ask about conversation history
                history_choice = Prompt.ask(
                    "[bold cyan]Conversation history? (a=5, b=10, c=20, d=70, e=none)[/bold cyan]",
                    default="e"  # ðŸ”§ Default to NONE for fresh roadmap queries
                ).strip().lower()

                history_map = {'a': 5, 'b': 10, 'c': 20, 'd': 70, 'e': 0}
                history_limit = history_map.get(history_choice, 0)
                ...
                self.console.print(
                    f"[dim]Using: {mode_display[doc_mode]} + "
                    f"{'no history' if history_limit == 0 else f'last {history_limit} conversations'}[/dim]"
                )

                # Show animated agent processing
                self.show_agent_progress(message)
                await self.process_message_with_progress(message, doc_mode, history_limit)
    ...
    async def run_interactive(self):
        ...
        self.display_banner()
        self.console.print("\n")
        with Status("[bold cyan]ðŸ”„ Starting multi-agent assistant...[/bold cyan]", console=self.console, spinner="dots"):
            self.assistant.start()
            await asyncio.sleep(1)
        self.console.print("[bold green]âœ“ LAZYCOOK is now running! [/bold green]\n")
        self.display_help()

        try:
            while True:
                self.console.print()
                command = Prompt.ask("[bold blue]ðŸ“ Enter command[/bold blue] (or 'quit' to exit)").strip().lower()
            ...
            with Status("[bold cyan]ðŸ”„ Shutting down multi-agent assistant...[/bold cyan]", console=self.console,
                        spinner="dots"):
                self.assistant.stop()
                await asyncio.sleep(1)
            self.console.print("\n[bold green]âœ“ Multi-Agent Assistant stopped successfully![/bold green]")
...
class MultiAgentAssistantConfig:
    ...
    def create_assistant(self):
        ...
        assistant.file_manager = EnhancedTextFileManager(
    ...
    def create_cli(self):
        ...
        cli.assistant.file_manager = EnhancedTextFileManager(
            conversation_limit=self.conversation_limit
        )
        return cli
    ...
    async def run_cli(self):
        ...
        try:
            ...
            import traceback
            console = Console()
            console.print(f"[bold red]âŒ Error: {str(e)}[/bold red]")
...
def main():
    ...
    if not api_key:
        console.print("[bold red]âŒ API key is required to run the assistant.[/bold red]")
        return

    with console.status("[bold cyan]ðŸ”„ Testing Gemini API connection...[/bold cyan]"):
        try:
            genai.configure(api_key=api_key)
            test_model = genai.GenerativeModel('gemini-1.5-flash')
            test_response = test_model.generate_content("Hello")
        except Exception as e:
            console.print(f"[bold red]âŒ API connection failed: {e}[/bold red]")
            return
