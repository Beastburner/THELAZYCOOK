...
class TextFileManager:
    ...
    def get_conversation_context(self, user_id: str, limit: int = None) -> str:
        ...
        # ✅ ADD DEBUG LOGGING:
        logger.info(f"DEBUG: get_conversation_context called with limit={limit}")
        logger.info(f"DEBUG: self.conversation_limit={getattr(self, 'conversation_limit', 'NOT SET')}")
    ...
    def get_session_conversations(self, user_id: str, limit: int = None) -> List[Conversation]:
        ...
        # ✅ ADD:
        logger.info(f"DEBUG: get_session_conversations limit={limit}")
    ...
    def get_recent_conversations(self, user_id: str, limit: int = None) -> List[Conversation]:
        ...
        # ✅ ADD:
        logger.info(f"DEBUG: get_recent_conversations limit={limit}")
...
# --- AI Agent ---
class AIAgent:
    def __init__(self, api_key_gemini: str, api_key_grok: str, role: AgentRole, temperature: float = 0.7):
        self.role = role
        self.temperature = temperature
        self.max_tokens = 4000

        # Configure based on role
        if role in [AgentRole.GENERATOR, AgentRole.ANALYZER]:
            # Use Gemini
        ...
        else:  # OPTIMIZER, VALIDATOR
            # Use Grok
            self.client = Groq(api_key=api_key_grok)
            self.model_name = "llama-3.3-70b-versatile"  # 128K context window
            self.use_gemini = False

    def _clean_grok_response(self, response_text: str) -> str:
        """
        ✅ ROBUST CLEANER: Remove ALL JSON/Markdown artifacts from Grok
        """
        import re
        import json
        ...
        # Step 2: Try to extract from JSON if it's still JSON
        try:
            data = json.loads(response_text)
            # Extract the actual content field
            if isinstance(data, dict):
                response_text = data.get('optimized_solution',
                                         data.get('content',
                                                  str(data)))
        ...
        # Step 3: Remove unicode control characters (but keep newlines/tabs)
        # This removes problematic characters while preserving formatting
        response_text = re.sub(r'[^\S\n\t\r]+', ' ', response_text)  # Normalize whitespace

        # Step 4: Fix escaped characters
        response_text = response_text.replace('\\n', '\n').replace('\\t', '\t')

        # Step 5: Remove JSON field names if they leaked through
        response_text = re.sub(r'"optimized_solution":\s*"?', '', response_text)
        response_text = re.sub(r'"content":\s*"?', '', response_text)
        response_text = re.sub(r'"changes_made":\s*\[', '', response_text)
        response_text = re.sub(r'"errors_fixed":\s*\[', '', response_text)
        response_text = re.sub(r'"enhancements":\s*\[', '', response_text)

        # Step 6: Remove trailing quotes/commas from JSON artifacts
        response_text = response_text.strip('",{}[]')

        # Step 7: Normalize line breaks (no triple+ newlines)
        response_text = re.sub(r'\n{3,}', '\n\n', response_text)

        # Step 8: Remove any remaining JSON array syntax
    ...
    async def _analyze_solution(self, user_query: str, context: str, previous_iteration: Dict) -> AgentResponse:
        ...
        # FOR ANALYZER (_analyze_solution method):
        try:
            if self.use_gemini:
                response = await self.model.generate_content_async(prompt)
            ...
            # Handle code blocks more reliably (FIXED: moved outside else block)
    ...
    Task: Improve the solution based on feedback and return structured data.

    USER QUERY: {user_query}

    ORIGINAL SOLUTION (COMPLETE):
    {original_solution}
    ...
    CRITICAL: Respond with valid JSON. In the optimized_content field, use \\n for newlines, not actual newlines.
    {{
      "optimized_content": "your improved solution here (use \\\\n for line breaks)",
      "changes_made": ["specific change 1", "specific change 2"],
      "errors_fixed": ["error 1 fixed", "error 2 fixed"],
    ...
    """
        ...
        logger.info(f"📊 Optimizer prompt: {len(prompt)} chars (~{len(prompt) // 4} tokens)")
        ...
        try:
            ...
            # ✅ FIX 1: Try direct parsing first
            try:
                data = json.loads(response_text)
                logger.info("✅ JSON parsed successfully (direct)")

            except json.JSONDecodeError as e1:
                logger.warning(f"⚠️ Direct JSON parse failed: {e1}")

                # ✅ FIX 2: Try with strict=False (allows control chars)
                try:
                    data = json.loads(response_text, strict=False)
                    logger.info("✅ JSON parsed successfully (strict=False)")

                except json.JSONDecodeError as e2:
                    logger.warning(f"⚠️ Strict=False parse failed: {e2}")

                    # ✅ FIX 3: Manual extraction as last resort
                    try:
                        # Extract optimized_content between quotes
                        content_match = re.search(r'"optimized_content"\s*:\s*"(.*?)"\s*,\s*"changes_made"',
                                                  response_text, re.DOTALL)
                        if content_match:
                            optimized_content = content_match.group(1)
                            # Unescape common escapes
                            optimized_content = optimized_content.replace('\\n', '\n')
                            optimized_content = optimized_content.replace('\\t', '\t')
                            optimized_content = optimized_content.replace('\\"', '"')
                        else:
                            optimized_content = original_solution

                        # Try to extract arrays
                        changes = re.search(r'"changes_made"\s*:\s*\[(.*?)\]', response_text, re.DOTALL)
                        fixes = re.search(r'"errors_fixed"\s*:\s*\[(.*?)\]', response_text, re.DOTALL)
                        enhance = re.search(r'"enhancements"\s*:\s*\[(.*?)\]', response_text, re.DOTALL)

                        changes_made = [s.strip(' "') for s in changes.group(1).split(',')] if changes else [
                            "Improved quality"]
                        errors_fixed = [s.strip(' "') for s in fixes.group(1).split(',')] if fixes else []
                        enhancements = [s.strip(' "') for s in enhance.group(1).split(',')] if enhance else []

                        logger.info("✅ JSON extracted manually (regex fallback)")
                        ...
                        # Ultimate fallback
                        optimized_content = original_solution
                        changes_made = ["Optimization attempted but parsing failed"]
                        errors_fixed = []
                        enhancements = []

            # If direct/strict parsing worked, extract data
            if 'data' in locals():
                optimized_content = data.get("optimized_content", original_solution)
            ...
            logger.info(
                f"✅ Final: {len(changes_made)} changes, {len(errors_fixed)} fixes, {len(enhancements)} enhancements")
...
class MultiAgentSystem:
    ...
    def __init__(self, api_key_gemini: str, api_key_grok: str):
...
class RichMultiAgentCLI:
    def __init__(self, gemini_api_key: str, grok_api_key: str):
        self.console = Console()
        self.assistant = AutonomousMultiAgentAssistant(gemini_api_key, grok_api_key)
    ...
    def create_assistant(gemini_api_key: str, grok_api_key: str, conversation_limit: int = 70, document_limit: int = 2):
        """Factory function to create a configured assistant for external use"""
        return MultiAgentAssistantConfig(gemini_api_key, grok_api_key, conversation_limit, document_limit)
    ...
    def display_response(self, user_message: str, ai_response: str, processing_time: float, context_used: str = ""):
        ...
        def final_clean(text):
            """Last-resort cleaner for any remaining JSON artifacts"""
        ...
        ai_response = final_clean(ai_response)
        ...
        if conversations and conversations[0].multi_agent_session:
            ...
            topics = conversations[0].topics

        # 1. User Message Panel
        user_panel = Panel(
        ...
        # 2. AI Response - Render as Markdown (handles formatting automatically)
        try:
            # ✅ FIX: Let Rich Markdown handle ALL formatting
            ...
            # Fallback: Display as plain text with preserved formatting
            logger.warning(f"Markdown rendering failed: {e}")

            response_panel = Panel(
                Text(ai_response, style="white", justify="left"),
                title=Text("🤖 Assistant Response", style="bold green"),
                border_style="green",
                padding=(1, 2),
                expand=True
    ...
    async def handle_chat_mode(self):
        ...
        while True:
            ...
            if message.strip():
                ...
                context = self.assistant.file_manager.get_conversation_context(self.user_id)
                self.console.print(
...
class MultiAgentAssistantConfig:
    ...
    def create_cli(self):
        ...
        # Replace BOTH the assistant's file_manager AND any cached contexts
        cli.assistant.file_manager = file_manager
        cli.assistant._cached_context = {}  # Clear any cached context
...
def main():
    ...
    if not gemini_api_key or not grok_api_key:
        console.print("[bold red]❌ Both API keys required[/bold red]")
        return

    # Test both APIs...
    cli = RichMultiAgentCLI(gemini_api_key, grok_api_key)
    try:
